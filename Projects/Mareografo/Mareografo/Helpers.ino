// to check if the arduino is ready to be used.
void getFonaReady(){
    checkpoint:
    bool isReady = 0;
        ...
        *bufPtr = fona.read();
        ...
        if (currentWrite != "\n") {
            ...
            if(readyness.indexOf("SMS Ready") > -1) {
                ...
                if (!fona.enableGPRS(true)){ // enable gprs
//                    Serial.println(F("Falha ao ligar o GPRS."));
                } else {
//                    Serial.println(F("GPRS Ligado com sucesso."));
                }

                if (!fona.enableNetworkTimeSync(true)) { // enable network time synchronisation
