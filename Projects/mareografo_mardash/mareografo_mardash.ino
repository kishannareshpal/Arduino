...
// Observations:
// takes ~16s to send sensor_data to ardash
...
void testTraccar(){
    char server_ip[] = "ardash.herokuapp.com";
    int server_port = 80;

    if(!gprs.connect(TCP, server_ip, server_port)) {
      Serial.println("Error connecting to ARDASH\n");
    }else{
      Serial.println("Connection to ARDASH Estab.\n");
    }
}
...
bool read_sendDist(){
  ...
  return sendData("d", String(dist_CM), "", "", "");
}
