...
String set_http_header(String data_type, String data_value, String temp_value, String hum_value, String dist_value){
    ...
    if((data_type=="") || (data_value=="")){
           ...
           h_head =  "PUT /?id=944425";
           h_head += "&lat=-23.868312";
           h_head += "&lon=35.376653";
           h_head += "&" + data_type + "=";
           h_head += data_value;
           h_head += " HTTP/1.0\r\n\r\n";
           return h_head;
...
void read_sendDist(){
  ...
  sendData("dist", String(dist_CM), "", "", "");
}

// Send data from all sensors every 1sec, for 10sec
void read_sendAll(){
    // First, open connection to Traccar:
    testTraccar();
    delay(2000);
    ...
    bool sent = sendData("", "", String(t), String(h), String(dist_CM));

    if(sent){
      gprs.sendSMS(string2char(numb), "Data sent successfully!");
    }
    received = false;
}
